...
def pricingsolver(pione, pitwo, sigma1):
    pia = []
    ...
    return reducedcost, pric.X

[redcost, newv] = pricingsolver(pi1, pi2, sigma)
...
while iter < 1000:
    if redcost <= 0:
        ...
        print("Convert it back to minimization problem, objval is: ", -rmpobjval)
        break
    elif redcost > 0:
        print("Need to resolve with new lambda!")
        print("Current objval is: ", rmpobjval)
        print("New extreme point is ", newv)

        extrpoint.append(newv)
        [cv, ch, av, ar, pi1, pi2, sigma, rmplbd, rmpobjval] = RMPsolve(extrpoint, extrrays)
        [redcost, newv] = pricingsolver(pi1, pi2, sigma)
        print("New reduced cost is ", redcost)
...
print("The final optimal solution is ", -rmpobjval)
print("Recall that zLP = 662.974,  Z_MIP = 1120, zLD = 1014, which is within some levels of tolerance.")